...
mixin template DDParserSupport() {
    ...
    struct DDTokenStream {
        ...
        {
            tokens = ddLexicalAnalyserSpecification.input_token_range(text, label);
            get_next_token();
        }
        ...
        void get_next_token()
        {
            ...
            currentTokenAttributes.ddMatchedText = mr.matched_text;
            if (mr.is_valid_match) {
                currentToken = mr.handle;
                dd_set_attribute_value(currentTokenAttributes, currentToken, mr.matched_text);
            } else {
